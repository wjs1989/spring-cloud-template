...
public class DefaultReactiveElasticsearchClient implements ReactiveElasticsearchClient {
    private Logger log = LoggerFactory.getLogger(DefaultReactiveElasticsearchClient.class);
    ...
    Request convertPutIndexTemplateRequest(PutIndexTemplateRequest putIndexTemplateRequest) {
        ...
        BytesRef source = null;
        try {
            source = XContentHelper.toXContent(putIndexTemplateRequest, XContentType.JSON, false).toBytesRef();
        } catch (IOException e) {
            throw new RuntimeException(e.getMessage());
        }
