...
import com.elasticsearch.index.DefaultElasticSearchIndexMetadata;
...
public abstract class AbstractElasticSearchIndexStrategy implements ElasticSearchIndexStrategy {
    private Logger log = LoggerFactory.getLogger(AbstractElasticSearchIndexStrategy.class);
    ...
    protected AbstractElasticSearchIndexStrategy(String id, ReactiveElasticsearchClient client, ElasticSearchIndexProperties properties) {
        this.id = id;
        this.client = client;
    ...
    protected Mono<Void> doCreateIndex(ElasticSearchIndexMetadata metadata) {
        return client.createIndex(createIndexRequest(metadata)).then();
    }
