...
import com.elasticsearch.index.ElasticIndex;
...
public interface ElasticSearchService {
    ...
    default <T> Flux<T> query(String index, QueryParam queryParam, Class<T> type) {
        return query(index, queryParam, map -> BeanUtils.copyProperties(map, type));
    }

    default <T> Mono<PagerResult<T>> queryPager(String index, QueryParam queryParam, Class<T> type) {
        return queryPager(index, queryParam, map -> BeanUtils.copyProperties(map, type));
    }

    default <T> Mono<PagerResult<T>> queryPager(ElasticIndex index, QueryParam queryParam, Class<T> type) {
        return queryPager(index.getIndex(), queryParam, map -> BeanUtils.copyProperties(map, type));
    }
    ...
    default <T> Flux<T> query(ElasticIndex index, QueryParam queryParam, Class<T> type) {
        return query(index.getIndex(), queryParam, map -> BeanUtils.copyProperties(map, type));
    }
